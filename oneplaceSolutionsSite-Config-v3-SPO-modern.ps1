<#  
    ...
    This script optionally creates a new Site collection ('Team Site (Modern)' by default, 'Team Site (Classic)' by option), and applies the configuration changes / PnP template for the OnePlace Admin Site.
    All major actions are logged to 'OPSScriptLog.txt' in the user's or Administrators Documents folder, and it is uploaded to the Admin Site at the end of provisioning.
#>
...
Write-Host 'Welcome to the Admin Site Deployment Script for OnePlace Solutions' -ForegroundColor Green
Write-Host "`n--------------------------------------------------------------------------------`n" -ForegroundColor Red
...
Try {
    ...
    Try {
        ...
        function Deploy ($createSite) {
            ...
            #Stage 1a Creating a Admin Site from scratch
            If($createSite) {
                $stage = "Stage 1/2 - Team Site (Modern) creation"
                Write-Host "`n$stage`n" -ForegroundColor Yellow
                Write-Progress -Activity "Admin Site Deployment" -CurrentOperation $stage -PercentComplete (33)
                ...
                Catch {
                    ...
                    If ($exMessage -like "*(403)*") {
                        Write-Log -level Error -Message "Error connecting to '$adminSharePoint'. Please ensure you have sufficient rights to create Site Collections in your Microsoft 365 Tenant. `nThis usually requires Global Administrative rights, or alternatively ask your SharePoint Administrator to perform the Admin Site Setup."
                ...
                Write-Log -Level Info -Message "Admin Site URL suffix set to $script:solutionsSite"
                ...
                Try {
                    ...
                    $filler = "Creating site collection with URL '$SolutionsSiteUrl' for the Admin Site, and owner '$ownerEmail'. This may take a while, please do not close this window, but feel free to minimize the PowerShell window and check back in 10 minutes."
                    Write-Host $filler -ForegroundColor Yellow
                    Write-Log -Level Info -Message $filler
            ...
            #Stage 1b Skipping Site Creation, identifying Admin Site instead
            Else {
                Write-Host "`nStage 1/2 - Identify Admin Site`n" -ForegroundColor Yellow
            ...
            Try {
                Write-Host "`nStage 2/2 - Apply Admin Site template`n" -ForegroundColor Yellow
                ...
                If(-not (Test-Path $Script:templatePath)) {
                    ...
                    #Download OnePlace Admin Site provisioning template
                    ...
                    Write-Log -Level Info -Message "Downloading OPS logo for Admin Site"
                }
                Else {
            ...
            Try {
                ...
                Write-Log -Level Info -Message "Admin Site URL = $SolutionsSiteUrl"
                if( $RecordLicenseListID ) {
                ...
                #Upload log file to Admin Site

                Add-PnPfile -Path $script:LogPath -Folder "Shared Documents"

                Write-Progress -Activity "Completed" -Completed

                Write-Host "`nPlease record the OnePlace Admin Site URL for usage in the OnePlaceMail Desktop and OnePlaceDocs clients. " -ForegroundColor Yellow
                ...
                If($true -eq $legacyLicensing) {
                    $importants = "Admin Site URL = $SolutionsSiteUrl`nLicense List URL   = $LicenseListUrl`nLicense List ID    = $licenseListId"
                }
                Else {
                    $importants = "Admin Site URL = $SolutionsSiteUrl"
                }
                Write-Host $importants

                Write-Host "`n-------------------`n" -ForegroundColor Red
                
                Write-Host "Opening Admin Site at $SolutionsSiteUrl..." -ForegroundColor Yellow
                Pause
        ...
        function showMenu { 
            ...
            Write-Host "1: Deploy the Admin Site template to an existing Group or Modern Team Site Collection at '/sites/oneplacesolutions'"
            Write-Host "2: Create a new Modern Team Site Collection at '/sites/oneplacesolutions' automatically and deploy the Admin Site template"
