<#
    This script applies the configuration changes for the OnePlace Solutions Administration Site.
...
Try { 
    ...
    function Send-OutlookEmail ($attachment,$body) {
        Try {
            ...
            $Mail.Subject = "Administration Site and License List Information generated by $from"
            $Mail.Body = "Hello Customer Success Team`n`nPlease find our Administration Site and License List details below:`n`n$body"
    ...
    Try {
        ...
        $stage = "Stage 2/2 - Apply Administration Site template"
        Write-Host "`n$stage`n" -ForegroundColor Yellow
        ...
        If(-not (Test-Path $PathImage )) {
            $filler = "Downloading OPS logo for Administration Site"
            Write-Host $filler -ForegroundColor Yellow 
            Write-Log -Level Info -Message $filler
        ...
        Write-Host "`nPlease record the OnePlace Solutions Administration Site URL and License Location / License List URL for usage in the OnePlaceMail Desktop and OnePlaceDocs clients, and the License List Id for the licensing process. " -ForegroundColor Yellow
    ...
    Catch [Microsoft.SharePoint.Client.ServerException] {
        ...
        If($exMessage -match 'A site already exists at url') {
            ...
            If($solutionsSite -ne 'oneplacesolutions'){
                Write-Host "Please run the script again and choose a different Administration Site suffix." -ForegroundColor Red
            }
        }
        Else {
            Throw $_
