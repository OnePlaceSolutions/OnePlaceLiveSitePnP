<#
    This script applies the configuration changes for the OnePlace Admin Site.
...
Try { 
    ...
    function Send-OutlookEmail ($attachment,$body) {
        Try {
            ...
            $Mail.Subject = "Admin Site and License List Information generated by $from"
            $Mail.Body = "Hello Customer Success Team`n`nPlease find our Admin Site and License List details below:`n`n$body"
    ...
    Try {
        ...
        $stage = "Stage 2/2 - Apply Admin Site template"
        Write-Host "`n$stage`n" -ForegroundColor Yellow
        ...
        If(-not (Test-Path $PathImage )) {
            $filler = "Downloading OPS logo for Admin Site"
            Write-Host $filler -ForegroundColor Yellow 
            Write-Log -Level Info -Message $filler
        ...
        Write-Host "`nPlease record the OnePlace Admin Site URL and License Location / License List URL for usage in the OnePlaceMail Desktop and OnePlaceDocs clients, and the License List Id for the licensing process. " -ForegroundColor Yellow
    ...
    Catch [Microsoft.SharePoint.Client.ServerException] {
        ...
        If($exMessage -match 'A site already exists at url') {
            ...
            If($solutionsSite -ne 'oneplacesolutions'){
                Write-Host "Please run the script again and choose a different Admin Site suffix." -ForegroundColor Red
            }
        }
        Else {
            Throw $_
